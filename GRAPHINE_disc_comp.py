"""
File used to compute circuit compilation for Graphine based methods
"""
import copy
import numpy as np
import math
...
class AODGrid:
    ...
    def __init__(self, grid_dims, array_dims):
        ...
        for row in range(grid_dims[0]):
            ...
            # If it's the last row, position it half the spacing away from the max Y dimension or adjust to fit within
        ...
        for col in range(grid_dims[1]):
            ...
            # If it's the last column, position it half the spacing away from the max X dimension or adjust to fit within
    ...
    def process_interference(self, curr_layer, gate_lists):
        ...
        while index < len(curr_layer):
            ...
            if len(gate_of_interest) == 2:
                ...
                increment_gate = True
                # Iterating in reverse order from end of the list to current gate
            ...
            for gate in curr_layer:
                if len(gate) == 2:
                    ...
                    if q_dist > self.radius:
                        self.swap_counts += math.floor(q_dist/self.radius) * 2
