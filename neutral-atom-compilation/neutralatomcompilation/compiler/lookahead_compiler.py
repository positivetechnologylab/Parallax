...
class LookaheadCompiler(Compiler):
    ...
    def _schedule_and_route(self,
        ...
        #print(self.hardware.qiskit_qubits)
        ...
        while len(frontier) > 0:
            ...
            for op in to_execute:
                if isinstance(op, SwapOp):
                    ...
                    #print("OP",op.qubits,q,l2p[q],l2p[q].qiskit_qubit)
                    if hasattr(op.op, 'to_mutable'):
                        # Get the method
                        #print("here")
                        op.op = op.op.to_mutable()
                    #print(op.op._label, "-------", str(op.index),"----------",dir(op.op),'-----',dir(op))
        ...
        logic_phys_map = []
        for q in l2p.keys():
            logic_phys_map.append((l2p[q], l2p[q].qiskit_qubit))
        return output_circuit, l2p, p2l, logic_phys_map #end schedule/route
