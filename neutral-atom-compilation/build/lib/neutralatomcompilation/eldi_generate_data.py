...
for key, value in comp_circs.items():
    # Remove the './benchmarks_retrans/' part from the key
...
for circ in circs.keys():
    ...
    # Reverse the mapping to get old key -> new key
    old_to_new_mapping = {old_key: new_key for new_key, old_key in enumerate(sorted(atom_locs.keys()))}

    # Function to replace old keys with new keys in the QASM string
    def replace_keys(match):
        old_key = int(match.group(1))  # Match group 1 is the number inside brackets
        return f"[{old_to_new_mapping.get(old_key, old_key)}]"  # Default to old_key if not in mapping

    # Regular expression pattern to match the old keys in brackets in the QASM string
    pattern = r'\[(\d+)\]'  # Matches numbers inside square brackets

    # Replace old keys in the QASM string
    qasm_string = re.sub(pattern, replace_keys, qasm_string)
    
    # Regular expression pattern to match 'q' followed by digits and then '['
    pattern = r'q\d+\['
    # Replace the matched pattern with 'q['
    ...
    # Rebuild the string without 'swap' lines
    new_qasm_string = ""
    for line in lines:
        if 'swap' in line:
            swap_count += 1
    ...
    # Format the intro string with the value of intro_ind
    intro = f"OPENQASM 2.0;\ninclude \"qelib1.inc\";\nqreg q[{intro_ind}];\n"

    # Insert the formatted intro string at the beginning of qasm_string
    qasm_string = intro + qasm_string
    qasms[circ] = [qasm_string,swap_count]

data_dict = {}
for key in qasms.keys():
    data_dict[key] = [new_atom_locs_dict[key],qasms[key]]#[dict_of_ind_to_coords,[qasmstr, num_swaps_removed]]

if HDWR == 'Atom':
    file_path = "baker_res_atom.pkl"
elif HDWR == 'Quera':
    file_path = "baker_res_quera.pkl"
    
# Saving the dictionary to a pickle file
with open(file_path, 'wb') as file:
    pickle.dump(data_dict, file)
    
print("Done creating aggregated .pkl file for "+HDWR+" hardware (non-parallelized circtuits)")
    
    
"""
------------
Begin Parallelized Circuit Data Generation
------------
"""
...
for circ in circs.keys():
    ...
    #Extract locations from inds
    parts = circ.split('_')  # Split the string by '_'
    dim_sz = int(parts[-1])  # Convert the last element to integer
...
for key in qasms.keys():
    # Splitting the key to extract required parts
    initial_string, num1, num2 = key.rsplit('_', 2)
    initial_string = initial_string.rsplit('.', 1)[0]  # Removing '.qasm'
    
    # Forming directory and filename
    directory_path = f'./par_results/{initial_string}'
    ...
    # Preparing the data to be pickled
    data_to_pickle = [new_atom_locs_dict[key], qasms[key]]
    
    # Pickle dumping the data
